...
import numpy as np
from IntelligentAssistantWriting.classification.entity_network import EntityNetwork
...
tf.app.flags.DEFINE_string("ckpt_dir",os.path.join(conf.dir_root,"checkpoint_entity_network2/"),"checkpoint location for the model")
tf.app.flags.DEFINE_integer("sequence_length",200,"max sentence length") #100
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word, vocabulary_index2Vector = create_voabulary(word2vec_model_path=FLAGS.word2vec_model_path,name_scope="entity_network") #simple='simple'
        ...
        if FLAGS.multi_label_flag:
            FLAGS.traning_data_path=os.path.join(conf.dir_root,"features_lexileLevel_text_all") #train
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                assign_pretrained_word_embedding(sess, vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index, vocab_size, model,word2vec_model_path=FLAGS.word2vec_model_path)
...
def assign_pretrained_word_embedding(sess, vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index,vocab_size,model,word2vec_model_path=None):
