...
import numpy as np
from IntelligentAssistantWriting.classification.BiLstmTextRelation_mode import BiLstmTextRelation
from IntelligentAssistantWriting.classification.data_util import load_data,create_voabulary ,create_voabulary_label
...
tf.app.flags.DEFINE_string("ckpt_dir",os.path.join(conf.dir_root,"biLstm_text_relation_checkpoint/"),"checkpoint location for the model")
...
tf.app.flags.DEFINE_string("traning_data_path",os.path.join(conf.dir_root,"train_test_data_fastText"),"path of traning data.") #train-zhihu4-only-title-all.txt===>training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'
tf.app.flags.DEFINE_string("word2vec_model_path",os.path.join(conf.dir_root,"w2v_model.model"),"word2vec's vocabulary and vectors")
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word, vocabulary_index2Vector = create_voabulary(simple='simple',word2vec_model_path=FLAGS.word2vec_model_path,name_scope="biLstmTextRelation")
        ...
        train, test, _= load_data(vocabulary_word2index, vocabulary_word2index_label,valid_portion=0.05,training_data_path=FLAGS.traning_data_path)
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                assign_pretrained_word_embedding(sess,vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index, vocab_size, biLstmTR,
...
def assign_pretrained_word_embedding(sess,vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index,vocab_size,textRNN,word2vec_model_path=None):
...
def do_eval(sess,biLstmTR,evalX,evalY,batch_size,vocabulary_index2word_label):
    ...
    eval_loss,eval_acc,eval_counter=0.0,0.0,0.000000000000000001
    # print(0,number_examples,batch_size)
