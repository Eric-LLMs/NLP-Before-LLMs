...
import numpy as np
from IntelligentAssistantWriting.classification.TextCNN_model import TextCNN
from IntelligentAssistantWriting.classification.data_util import load_data_multilabel_new,create_voabulary,create_voabulary_label
...
tf.app.flags.DEFINE_string("ckpt_dir",os.path.join(conf.dir_root,"text_cnn_title_desc_checkpoint/"),"checkpoint location for the model")
tf.app.flags.DEFINE_integer("sentence_len",200,"max sentence length")
tf.app.flags.DEFINE_integer("embed_size",128,"embedding size")
tf.app.flags.DEFINE_boolean("is_training",True,"is traning.true:tranining,false:testing/inference")
tf.app.flags.DEFINE_integer("num_epochs",102,"number of epochs to run.")
...
tf.app.flags.DEFINE_string("traning_data_path",os.path.join(conf.dir_root,"features_lexileLevel_text_all"),"path of traning data.") #O.K.train-zhihu4-only-title-all.txt-->training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'
tf.app.flags.DEFINE_integer("num_filters", 256, "number of filters") #256--->512
tf.app.flags.DEFINE_string("word2vec_model_path",os.path.join(conf.dir_root,"w2v_model.model"),"word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100-->zhihu-word2vec-multilabel-minicount15.bin-100
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word,vocabulary_index2Vector = create_voabulary(word2vec_model_path=FLAGS.word2vec_model_path,name_scope="cnn2") #simple='simple'
        ...
        if FLAGS.multi_label_flag:
            FLAGS.traning_data_path=os.path.join(conf.dir_root,"features_lexileLevel_text_all") #test-zhihu5-only-title-multilabel.txt
    ...
    with tf.Session(config=config) as sess:
        ...
        textCNN=TextCNN(filter_sizes,FLAGS.num_filters,FLAGS.num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps,
                        FLAGS.decay_rate,FLAGS.sentence_len,vocab_size,FLAGS.embed_size,FLAGS.is_training,multi_label_flag=FLAGS.multi_label_flag)#initializer='' ,
        #Initialize Save
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                assign_pretrained_word_embedding(sess, vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index, vocab_size, textCNN,word2vec_model_path=FLAGS.word2vec_model_path)
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                # print('******************************************************************************************')
                # print(sess.run(tf.nn.embedding_lookup(textCNN.Embedding, [vocabulary_word2index['A'],vocabulary_word2index['healthy'],vocabulary_word2index['diet']])))
                # print(sess.run(textCNN.Embedding[vocabulary_word2index['A']]))
                # print(sess.run(textCNN.Embedding[vocabulary_word2index['healthy']]))
                # print(vocabulary_word2index['A'],vocabulary_word2index['healthy'],vocabulary_word2index['diet'])
                # print('******************************************************************************************')
...
def assign_pretrained_word_embedding(sess,vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index,vocab_size,textCNN,word2vec_model_path=None):
    ...
    for word  in word2vec_model.wv.vocab :
        # w_index = word2vec_model.wv.vocab[word].index
    ...
    # 顺序赋值20180329
    for k, v in vocabulary_index2Vector.items():
        word_embedding_final[k] = v
