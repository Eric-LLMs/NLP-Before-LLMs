...
import numpy as np
from IntelligentAssistantWriting.classification.fastText_model import fastText as fastText
...
tf.app.flags.DEFINE_string("ckpt_dir",os.path.join(conf.dir_root,"fast_text_checkpoint/"),"checkpoint location for the model")
...
tf.app.flags.DEFINE_string("cache_path",os.path.join(conf.dir_root,"fast_text_checkpoint/data_cache.pik"),"checkpoint location for the model")
...
def main(_):
    ...
    if 1==1:
        ...
        vocabulary_word2index, vocabulary_index2word ,vocabulary_index2Vector= create_voabulary()
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                assign_pretrained_word_embedding(sess, vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index, vocab_size,vocab_size,fast_text)
...
def assign_pretrained_word_embedding(sess,vocabulary_index2word,vocabulary_index2Vector,vocabulary_word2index,vocab_size,fast_text,word2vec_model_path=None):
    print("using pre-trained word emebedding.started...")
    # word2vecc=word2vec.load('word_embedding.txt') #load vocab-vector fiel.word2vecc['w91874']
    # word2vec_model = word2vec.word2vec.load('zhihu-word2vec-multilabel.bin-100', kind='bin')
